...
class Game:
            ...
            for player in ["X", "O"]:
                ...
                for j in range(len(rows) - self.s + 1):
                    ...
                    if(np.all(cols[j: j + self.s] == player_win)): # horizontal win
                        return player
                    if(len(diag_1) >= self.s and (j + self.s <= len(diag_1))): # diag 1 win
                        if(np.all(diag_1[j: j + self.s] == player_win)):
                            return player
                    if(len(diag_2) >= self.s and (j + self.s <= len(diag_1))): # diag 2 win
    ...
    def is_end_old(self):
        ...
        # Horizontal win
        for i in range(0, 3):
            if (np.any(self.current_state[i] == ['X', 'X', 'X'])):
                return 'X'
            elif np.any(self.current_state[i] == ['O', 'O', 'O']):
                return 'O'
        # Main diagonal win
    ...
    def heuristic1(self, max, value, algo):
        ...
        """
        center_board = np.ceil(self.n/2).astype(int)-1
        if max:
    ...
    def e1(self, max, algo=MINIMAX):
        '''Prioritize center board, then highest score based on number of adjacent friends/allies'''
        value = 2
        if max:
            value = -2
        x = None
        y = None
        result = self.is_end_old()
        if result == 'X':
            return (-1, x, y)
        elif result == 'O':
            return (1, x, y)
        ...
        if self.current_state[center_board][center_board] == '.':
            return (1 if max else -1, center_board, center_board)
        for i in range(self.n):
            for j in range(self.n):
                if self.current_state[i][j] == '.':
                    if max: # max, center board taken
                        scores = self.calculate_score("O")
                        max_i, max_j = np.unravel_index(np.argmax(scores), (self.n,self.n))
                        self.current_state[max_i][max_j] == 'O'
                        if algo == self.MINIMAX:
                            (v, _, _) = self.e1(max=False)
                            if v > value:
                                value = v
                                x = max_i
                                y = max_j
                        self.current_state[max_i][max_j] == '.'
                    else: # min, center board taken
                        scores = self.calculate_score("X")
                        max_i, max_j = np.unravel_index(np.argmax(scores), (self.n,self.n))
                        self.current_state[max_i][max_j] == 'X'
                        if algo == self.MINIMAX:
                            (v, _, _) = self.e1(max=True)
                            if v < value:
                                value = v
                                x = max_i
                                y = max_j
